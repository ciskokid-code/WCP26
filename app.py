...
# ── Bracket data (shared across pages) ───────────────────────────────────────
...
# ══════════════════════════════════════════════════════════════════════════════
# Navigation  —  pill bar, each pill is a link with ?tab=<key>
# ══════════════════════════════════════════════════════════════════════════════
...
for _key, _label in _NAV:
    _on  = _key == _active
    _bg  = "#ff6b35" if _on else "#1e1e2e"
    _col = "#fff"    if _on else "#888"
    _brd = "#ff6b35" if _on else "#2a2a3a"
    _nav_html += (
        f'<a href="?tab={_key}" style="text-decoration:none;flex-shrink:0;">'
        f'<div style="background:{_bg};color:{_col};border:1px solid {_brd};'
        f'border-radius:20px;padding:7px 16px;font-size:13px;font-weight:600;">'
        f'{_label}</div></a>'
    )
_nav_html += '</div>'
...
if _active == "bet":
    ...
    st.markdown(
        ...
        'Model pick for every match. Calibrated to historical WC averages '
        '(~2.7 goals/game) via Elo-ratio Poisson.</p>',
        unsafe_allow_html=True,
    )
    ...
    if _bp_choice != "Knockout":
        ...
        for _rnd_name, _pairs in _ko_rounds:
            ...
            for _ta, _tb in _pairs:
                ...
                st.markdown(_score_card(_ta, _tb, _ea3, _eb3, label=_rnd_name),
                            unsafe_allow_html=True)

    st.caption("Score = single most likely exact result · Elo-ratio Poisson, calibrated to WC averages")


# ══════════════════════════════════════════════════════════════════════════════
# PAGE — Title Odds  (?tab=odds)
# ══════════════════════════════════════════════════════════════════════════════

elif _active == "odds":
    team_group = {t: g for g, teams in GROUPS.items() for t in teams}
    champ  = live_sim["champion"]
    ...
    for team, label in zip(ranked[:3], ["FAVOURITE", "SECOND FAVOURITE", "THIRD FAVOURITE"]):
        ...
        hero_html += (
            ...
            f'<div style="font-size:20px;font-weight:700;margin-bottom:2px;">{flag(team)} {team}</div>'
            f'<div style="font-size:40px;font-weight:800;color:#ff6b35;line-height:1.1;margin-bottom:4px;">'
            f'{p * 100:.1f}%</div>'
            f'<div style="font-size:11px;color:#555;">to lift the trophy · Group {g}</div>'
            f'</div>'
        )
    hero_html += "</div>"
    ...
    st.markdown(_hbar_list([(t, champ.get(t, 0)) for t in ranked], max_p), unsafe_allow_html=True)
    st.caption(f"Model: Elo + Poisson · {n_sims:,} simulations")


# ══════════════════════════════════════════════════════════════════════════════
# PAGE — Groups  (?tab=groups)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# PAGE — Match Predictor bracket  (?tab=bracket)
# ══════════════════════════════════════════════════════════════════════════════

elif _active == "bracket":
    _CW = 124
    _GW = 16
