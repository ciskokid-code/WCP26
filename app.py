...
def _hbar_list(teams_probs: list[tuple[str, float]], max_p: float, start_rank: int = 1) -> str:
    ...
    for i, (team, p) in enumerate(teams_probs, start_rank):
        ...
        html += (
            ...
            f'<div style="width:90px;background:#2a2a2a;border-radius:4px;height:8px;flex-shrink:0;overflow:hidden;">'
...
# ── Live scores (fetched once, used across sections) ──────────────────────────

_live_scores = _fetch_live_scores()

# ── Bet Predictor card helper ─────────────────────────────────────────────────
...
def _score_card(home: str, away: str, ea: float, eb: float,
    ...
    today_badge = (
        ...
        'background:#2a1000;padding:2px 8px;border-radius:10px;margin-left:6px;">TODAY</span>'
        if is_today else ""
    )
    live_badge = ""
    if is_active:
...
st.markdown(
    ...
    f'<h2 style="font-size:32px;font-weight:800;margin:0 0 6px 0;">🏆 Who wins the World Cup?</h2>'
...
st.markdown(
    ...
    'Bar = chance to win the group. Percentages show 1st / 2nd finish probability.</p>',
    unsafe_allow_html=True,
)
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 4 — Match Predictor bracket
# ══════════════════════════════════════════════════════════════════════════════
...
def _mcard(ta: str, tb: str) -> str:
    ...
    return (
        ...
        f'</div>'
        f'<span style="font-size:9px;font-weight:700;color:#ff6b35;flex-shrink:0;'
        f'margin-left:2px;">{wp:.0f}%</span>'
        f'</div>'
        f'<div style="display:flex;align-items:center;gap:3px;padding:5px 6px;">'
        f'<span style="font-size:13px;flex-shrink:0;">{flag(l)}</span>'
        f'<div style="flex:1;overflow:hidden;">'
        f'<div style="font-size:10px;color:#3a3a55;white-space:nowrap;overflow:hidden;'
        f'text-overflow:ellipsis;">{l[:13]}</div>'
...
def _conn(n: int) -> str:
    pairs = "".join(
        ...
        '<div style="flex:1;border-right:2px solid #2d2d4a;border-bottom:2px solid #2d2d4a;'
        'border-radius:0 0 3px 0;"></div>'
        '<div style="flex:1;border-right:2px solid #2d2d4a;border-top:2px solid #2d2d4a;'
...
st.markdown('<h2 style="margin-bottom:16px;">⚔️ Head-to-Head</h2>', unsafe_allow_html=True)
